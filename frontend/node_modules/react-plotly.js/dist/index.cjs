...
// src/react-plotly.js
var react_plotly_exports = {};
__export(react_plotly_exports, {
  default: () => react_plotly_default
});
module.exports = __toCommonJS(react_plotly_exports);
...
function plotComponentFactory(Plotly2) {
  return (0, import_react.forwardRef)(function PlotlyComponent({
    data = DEFAULT_DATA,
    layout,
    config,
    frames,
    ...
    function runPlotlyReact(shouldInvokeResize, figureCallback, shouldAttachUpdateEvents) {
      promiseRef.current = promiseRef.current.then(() => {
        ...
        return Plotly2.react(elRef.current, { data, layout, config, frames });
      }).then(() => {
        if (unmountingRef.current) {
          return;
        }
    ...
    function syncWindowResize(invoke) {
      ...
      if (useResizeHandler && !resizeHandlerRef.current) {
        resizeHandlerRef.current = () => Plotly2.Plots.resize(elRef.current);
    ...
    return /* @__PURE__ */ import_react.default.createElement("div", { id: divId, style, ref: setRef, className });
  });
}

// src/react-plotly.js
var import_plotly = __toESM(require("plotly.js/dist/plotly"));
var PlotComponent = plotComponentFactory(import_plotly.default);
var react_plotly_default = PlotComponent;
